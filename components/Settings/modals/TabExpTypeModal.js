...
const TabExpTypeModal = (props) =>{
	
	const {settings,settingsShows, updtSettings, selectValueSettings, displayDialogSettings,
		  setDisplayDialogSettings,setSettingsShows, valueSettings, setRedValid, expOwner} = useContext(SettingsContext);
	const {uidCollection} = useContext(AuthContext);
	
	const GroupOpt = expOwner.map((s,i)=>{
	...
	const handleSave= async(e) => { 
		let indx =  settings.exType!=null ? settings.exType.findIndex(x=>x.id===valueSettings.id) : -1;
		...
		if(indx!==-1){ //update, id is in the list 
			...
			for(let rw in settings.exType){
			if(settings.exType[rw].item.toLowerCase()===valueSettings.item.toLowerCase() &&
				   	settings.exType[rw].id!==valueSettings.id){
					exstIntheList=true;  // the expense exist and is valid, do not update/add	
				}else{
					newArr = settings.exType.map(x =>x.id===valueSettings.id ?
									  {...valueSettings}: x);
				}
			}
			...
			for(let rw in settings.exType){
			if(settings.exType[rw].item.toLowerCase()===valueSettings.item.toLowerCase() &&
				   	settings.exType[rw].show ){
			exstIntheList=true;  // the expense exists and is valid, do not update/add	
			}else if(settings.exType[rw].item.toLowerCase()===valueSettings.item.toLowerCase()
						 &&	!settings.exType[rw].show ){ //expense is deleted but in use
		 			newArr = settings.exType.map(x =>x.item.toLowerCase()===valueSettings.item.toLowerCase()
		...
		if(newArr.length!==0){ //just an update
			props.setSnackbar( {open: (await addDataSettings(uidCollection, 'settings', 'exType', {'exType':newArr})),
		 			  	msg: `Expense ${tmpValue.item} has been updated!`, variant: 'success'}); 
		}else{
			let tmpExType = settings.exType!=null ? settings.exType: [];
...
export default TabExpTypeModal;
