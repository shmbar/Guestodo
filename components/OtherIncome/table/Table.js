...
const tableCols = [
			...
			{field: 'PrpName', header: 'Property', showcol: true,s:['xs','sm','md','lg', 'xl']},
			...
			{field: 'IncAmntWthtoutVat', header: 'Amount Before Vat', showcol: true,s:['lg', 'xl']},
			...
			{field: 'Blnc', header: 'Balance Due', showcol: true,s:['md','lg', 'xl']},
...
const Table =() =>{
	...
	const {otherInc, setOtherInc, selectValue, displayDialog, setDisplayDialog,
	...
	const addOrder= ()=>{
	 ...
	 const createEmptyObj = () =>{
		...
		tmpObj.Transaction = '';
		 ...
		 tmpObj.owner = settings.properties.filter(x=> x.id===propertySlct)[0]['Owner']
		 tmpObj.Blnc='';
		 tmpObj.PmntStts='';
	...
	const selectValueOrder= (r)=>{
		let tmp = {...r, 'PrpName': settings.properties.filter(x=> x.PrpName===r.PrpName)[0]['id'],
	 			'incType': settings.incType.filter(y=>y.item===r.incType)[0]['id']}
	...
	const handleDelete=()=>{
		let newArr = otherInc.filter(q=>q.Transaction!==id);
			setOtherInc(newArr);
			async function Snack() {
				setSnackbar({open: ( await delData(uidCollection, 'otherIncome',date.year,  id)), msg: 'Income has been deleted!',
							 variant: 'success'}) ; 
				await delDPaymentsBatch(uidCollection,'payments', otherInc.filter(q=>q.Transaction===id)[0].Payments)
			}
		Snack();
		setOpen(false);
	}
	...
	const setSHortTr=(ddd) => { //for data export to excel
	 ...
	 const header = <Header 
                                ...
                                cols={cols}
                                tableData={setSHortTr(convId2Item(otherInc, ['incType','PrpName'], settings))}
								showOwners={false}
								showSearch={true}
                            /> ;  
	...
	return(	
		...
		<Grid container >
			...
			<DataTable  value={convId2Item(otherInc, ['incType','PrpName'], settings)}
