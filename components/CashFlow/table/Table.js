...
const tableCols = [
            {field: 'LstSave', header: 'Last Created', showcol: true,s:['xs','sm','md','lg', 'xl']},
...
const Table =(props) =>{
	...
	const {cfData, setCfData, selectValue, displayDialog, setDisplayDialog,
		  	snackbar, setSnackbar} = useContext(CfContext);
	const {fundSlct, date} = useContext(SelectContext);
	...
	const addOrder= ()=>{
		if(fundSlct===null){
			setSnackbar({open: true, msg: 'Choose fund',
						 variant: 'warning'});
			return;
		}else{
	 ...
	 const createEmptyObj = () =>{
		...
		tmpObj.owner = settings.funds.filter(x=> x.id===fundSlct)[0]['Owner'];
	...
	const handleDelete=()=>{
		let newArr = cfData.filter(q=>q.Transaction!==id);
			setCfData(newArr);
			async function Snack() {
				setSnackbar({open: ( await delData(uidCollection, 'cashflow', date.year,  id)), msg: 'Transaction has been deleted!',
							 variant: 'success'}) ; 
			}
		Snack();
		setOpen(false);
	...
	const showShortTR=(rowData, column) => {
	 ...
	 const header = <Header 
                                ...
                                cols={cols}
                                tableData={setSHortTr(convId2Item(cfData, ['PM'], settings))}
								showOwners={true}
								showSearch={true}
                            /> ;  
	...
	return(	
		...
		<Grid container >
			...
			<DataTable  value={convId2Item(cfData, ['PM'], settings)}
						ref={(el) => setDt(el)}
