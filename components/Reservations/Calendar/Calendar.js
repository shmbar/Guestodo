...
import {
	inCalendar,
	endCalendarNotLastDay,
	endCalendarLastDay,
	startCalendarNotEndFirstDay,
	startCalendarEndFirstDay,
	handleMouseDown,
	selectCells,
	handleMouseUp,
	cellsFuncSelect,
...
const Calendar = () => {
	...
	useEffect(() => {
		...
		for (let i = 0; i <= rsrcs.length; i++) {
			//Create an empty table of all dates
			if (rsrcs[i] != null) {
				let scheduleApt = arrTmp.reduce((o, key) => ({ ...o, [key]: null }), {});
				rcDataPrp
					.filter((q) => q.pStatus !== 'Cancelled')
					.filter((x) => x.AptName === rsrcs[i].id)
					.map((y, i) => {
						return getDatesBetweenDates(y.ChckIn, y.ChckOut)
							.filter((q) => dateFormat(q, 'mm') * 1 === date.month + 1)
							.map(
								(z) =>
									(scheduleApt[z.getDate()] =
										dateFormat(z, 'dd-mmm-yyyy') === y.ChckIn &&
										scheduleApt[z.getDate()] !== 'e'
											? 's'
											: dateFormat(z, 'dd-mmm-yyyy') === y.ChckIn &&
											  scheduleApt[z.getDate()] === 'e'
											? 'c'
											: dateFormat(z, 'dd-mmm-yyyy') === y.ChckOut &&
											  scheduleApt[z.getDate()] !== 's'
											? 'e'
											: dateFormat(z, 'dd-mmm-yyyy') === y.ChckOut &&
											  scheduleApt[z.getDate()] === 's'
											? 'c'
			...
			<tr key={i}>
				{schdl.map((scd, ind) => (
					<td
						...
						onMouseOver={(e) =>
							write && selectCells(ind, mouseDown, setSlctdCell, slctdCell)
						}
						onMouseUp={(e) =>
							write &&
							handleMouseUp(
					...
					>
						<div
							style={
								mouseDown
									? cellsFuncSelect(ind, i, slctdCell, schdl, resources, tableObj)
									: null
							}
						>
							{!loading &&
								rcDataPrp
									.filter((q) => q.pStatus !== 'Cancelled')
									.filter((z) => z.AptName === rs.id)
									.map((slot, index) => {
										return scd === dateFormat(slot.ChckIn, 'dd') * 1 &&
											getDate(slot.ChckIn) >=
												new Date(date.year, date.month, 1) &&
											getDate(slot.ChckOut) <=
												new Date(date.year, date.month + 1, 0) ? (
											<div
												key={index}
												onMouseEnter={(event) =>
													handlePopoverOpen(event, slot.Transaction)
												}
											...
											>
												...
												{openDtl === slot.Transaction && (
													<ShowDetails
														anchorEl={anchorEl}
														slot={slot}
														selectValueOrder={selectValueOrder}
													/>
												)}
											</div>
										  ...
										  getDate(slot.ChckIn) <
												new Date(date.year, date.month, 1) &&
										  getDate(slot.ChckOut).getDate() ===
												new Date(date.year, date.month, 1).getDate() ? (
											<div
												key={index}
												onMouseEnter={(event) =>
													handlePopoverOpen(event, slot.Transaction)
	...
	const selectValueOrder = async (slot) => {
		let slotsData = await readDataSlots(
			uidCollection,
			'slots',
			dateFormat(slot.ChckIn, 'yyyy'),
			null,
			slot.AptName
		);
	...
	const openModalRsrv = async () => {
		...
		let end;
		if (
			new Date(date.year, date.month, schdl[slctdCell.xFrst]) <
			new Date(date.year, date.month, schdl[slctdCell.xScnd])
		) {
		...
		let tmpObj = {
			ChckIn: dateFormat(strt, 'dd-mmm-yyyy'),
			ChckOut: dateFormat(end, 'dd-mmm-yyyy'),
			Transaction: 'RC'.concat(tmpRC).concat('_' + uuidv4()),
			Payments: [{ P: '', Date: null, PM: '', id: uuidv4() }],
			Vat:
				+settings.properties.filter(
					(x) =>
						x.id === settings.apartments.filter((x) => x.id === AptNAme)[0]['PrpName']
				)[0]['VAT'] === 0
					? false
					: true,
	...
	return (
		<div style={{ position: 'relative' }}>
			...
			{openAddRsrv && (
				<AddRsrvDialog
					setOpenAddRsrv={setOpenAddRsrv}
					openModalRsrv={openModalRsrv}
