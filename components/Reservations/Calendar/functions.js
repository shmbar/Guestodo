...
export const getCellWidth = (container, date) => {
	return (
		container.current.clientWidth / new Date(date.year, date.month + 1, 0).getDate()
	); /*+ (tfValue.substring(0, 1) - 3)*/
};
...
export const getSlotWidthL = (slot, date, cellWidth) => {
	return (
		cellWidth * getNights(slot.ChckOut, new Date(date.year, date.month, 1)) + cellWidth * 0.44
	);
};

const findImg = (txt, y, chnnlslogo) => {
	return chnnlslogo.filter((x) => x.brnd === txt)[0] !== undefined
		? chnnlslogo.filter((x) => x.brnd === txt)[0][y]
		: chnnlslogo[chnnlslogo.length - 1][y];
};
...
export const inCalendar = (settings, slot, chnnlslogo, index) => {
	return (
		<div
			...
			className="inTable"
			style={{
				width: getSlotWidth(slot, 60),
				borderLeftColor: PmntClrStatus1(slot.PmntStts),
				cursor: 'cell',
		...
		>
			<div className="event">
				<img
					src={findImg(
						idToItem(settings.channels, slot.RsrvChn, 'RsrvChn'),
						'img',
						chnnlslogo
					)}
					alt={slot.RsrvChn}
				...
				/>
				{getSlotWidth(slot, 60) - 45 > 50 && (
					<span style={{ width: getSlotWidth(slot, 60) - 45 }}>{slot.GstName}</span>
				)}
			</div>
		</div>
	);
...
export const endCalendarNotLastDay = (settings, slot, chnnlslogo, index, date) => {
	return (
		<div
			...
			className="inTable"
			style={{
				width: getSlotWidthR(slot, date, 60),
				borderLeftColor: PmntClrStatus1(slot.PmntStts),
				cursor: 'cell',
				backgroundColor: eventColor(slot),
			}}
		>
			<div className="event">
				...
				/>
				{getSlotWidthR(slot, date, 60) - 45 > 50 && (
					<span style={{ maxWidth: getSlotWidth(slot, 60) - 45 }}>{slot.GstName}</span>
				)}
			</div>
			<span className="styleEnd" style={{ zIndex: '-1' }}></span>
		</div>
	);
};
...
export const endCalendarLastDay = (settings, slot, chnnlslogo, index, date) => {
	return (
		...
		>
			<div style={{ backgroundColor: PmntClrStatus1(slot.PmntStts) }}></div>
		</div>
	);
};
...
export const startCalendarNotEndFirstDay = (settings, slot, chnnlslogo, index, date) => {
	...
	return (
		<div
			...
			style={{
				width: getSlotWidthL(slot, date, 60),
				cursor: 'cell',
			//	backgroundColor: eventColor(slot),
			}}
		>
			<div className="eventStart">
				...
				/>
				{getSlotWidthL(slot, date, 60) - 45 > 50 && (
					<span style={{ maxWidth: getSlotWidth(slot, 60) - 45 }}>{slot.GstName}</span>
				)}
			</div>
		</div>
...
export const startCalendarEndFirstDay = (slot, index, date) => {
	return (
		<div
			key={index}
			className="styleEndFirstDay"
			style={{ width: getSlotWidthL(slot, date, 60), cursor: 'cell' }}
		>
...
export const handleMouseDown = (
	...
	if (
		tableObj[resources[cellY].id][schdl[cellX]] !== null &&
		tableObj[resources[cellY].id][schdl[cellX]] !== 'e'
	)
		return;
...
export const handleMouseUp = (
	...
	if (mouseDown) {
		...
		if (slctdCell.xFrst === slctdCell.xScnd) {
			setSnackbar({
				open: true,
				msg: 'The selection shall include at least one night',
				variant: 'warning',
			});
			return;
		}
		...
		for (let i = strt; i <= end; i++) {
			if (
				tableObj[resources[slctdCell.y].id][i] === 't' ||
				tableObj[resources[slctdCell.y].id][i] === 'c' ||
				(tableObj[resources[slctdCell.y].id][i] === 's' &&
					tableObj[resources[slctdCell.y].id][i + 1] === 'e' &&
					i + 1 <= end)
			) {
				cancel = true;
				setSnackbar({
					open: true,
					msg: 'This apartment is already reserved for the selected dates',
					variant: 'warning',
				});
				return;
			}
...
export const cellsFuncSelect = (ind, i, slctdCell, schdl, resources, tableObj) => {
	if (
		(tableObj[resources[i].id][schdl[ind]] === 't' ||
			tableObj[resources[i].id][schdl[ind]] === 'c') &&
		i === slctdCell.y &&
		((ind >= slctdCell.xFrst && ind <= slctdCell.xScnd) ||
			(ind >= slctdCell.xScnd && ind <= slctdCell.xFrst))
	) {
		return style2;
	} else if (
		(tableObj[resources[i].id][schdl[ind]] === null ||
			tableObj[resources[i].id][schdl[ind]] === 's' ||
			tableObj[resources[i].id][schdl[ind]] === 'e') &&
		i === slctdCell.y &&
		((ind >= slctdCell.xFrst && ind <= slctdCell.xScnd) ||
			(ind >= slctdCell.xScnd && ind <= slctdCell.xFrst))
	) {
