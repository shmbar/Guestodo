...
const RsrvAmounts = () =>{
	...
	return (
			<Grid container spacing={3}>
					...
					<Grid item xs={12} style={{width:'100%'}}>
						<RowOut name='Reservation Amount Before Vat' value={value.NetAmnt!=='' ? 
							reservationAMountBeforeVat: ''} pad='0'/>
					</Grid>
					<Grid item xs={12} style={{width:'100%'}}>
						<RowOut name='Vat' value={value.NetAmnt!=='' ? vatAmount: ''} pad='0'/>
					</Grid>	
					
					<Grid item xs={12} style={{width:'100%'}}>
						<RowOut name={(value.NigthsNum!=='' && value.NetAmnt!=='') ? `Reservation Amount ${txt} X ${value.NigthsNum} Nights` : 	'Reservation Amount' } 
								value={value.NetAmnt!=='' ? ReservationAmount: ''} pad='0'/>
					</Grid>	
			
					{ChnCmsn!=='' &&
					<Grid item sm={12} style={{width:'100%'}}>
						<RowOut name={`Channel Service Fee (${ChnCmsn}%)`}
							value={value.NetAmnt!=='' ? `${cur} ${Num2(value.TtlRsrvWthtoutVat*ChnCmsn/100)}` :''}
							pad='0' />
					</Grid>
					}
					...
					<Grid item sm={12} style={{width:'100%'}}>
						<RowOut name={ChnCmsn!=='' ? 'Channel Payout' : 'Payout'} value={value.NetAmnt!=='' ? 
					...
					<Grid item sm={12} style={{width:'100%'}}>
						<RowOut name='Balance Due Reservation' value={(value.BlncRsrv!=='' && value.NetAmnt!=='') ?`${cur} ${Num2(+value.BlncRsrv)}`:''} pad='0' />
					</Grid>
					<Grid item sm={12} style={{width:'100%'}}>
