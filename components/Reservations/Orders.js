...
import {
	readDataPerPropertyDates,
	isSlotAvailale,
	readData,
	readDataMultiPropertyDates,
...
import TableTtl from '../Subcomponents/tableTitleAndExplainCalendar';

import './Orders.css';

const logos = [
	{ txt: 'Total', img: rsrv, width: '50px' },
...
export default function PaperSheet() {
	...
	useEffect(() => {
		const loadData = async () => {
			...
			if (!calendarView) {
				listDataRC = await readDataPerPropertyDates(
					uidCollection,
					'reservations',
					propertySlct,
					date.year,
				...
				listDataRC = await readDataMultiPropertyDates(
					uidCollection,
					'reservations',
					multiPropertySlct,
					date.year,
	...
	useEffect(() => {
		...
		for (let i = 0; i < rcDataPrp.length; i++) {
			nightsNum +=
				rcDataPrp[i].Transaction != null
					? rcDataPrp[i].pStatus === 'Tentative'
						? 0
						: +rcDataPrp[i].NigthsNum
					: 0;
		...
		setPnldata({
			avg: reservs !== 0 ? addCommas(reservs / nightsNum) : 0,
			ttlrsrv: addCommas(reservs),
			ttlblnxrsrv: addCommas(blnxrsrv),
		});
	}, [rcDataPrp]);
			...
			{!calendarView && (
				<div className={classes.paddingSmall}>
					<Grid container spacing={7} justifyContent="space-between">
						<PannelData
							clsNum="2"
							txt="Reservation Amount"
							ttl={
								date.month === 12
									? `Total ${date.year}`
									: dateFormat(new Date(date.year, date.month + 1, 0), 
												 'mmm-yyyy')
							}
							num={pnldata.ttlrsrv}
							img={logos[0]}
						...
						<PannelData
							clsNum="1"
							txt="Reservation Amount Per Night"
							ttl={
								date.month === 12
									? `Total ${date.year}`
									: dateFormat(new Date(date.year, date.month + 1, 0), 
