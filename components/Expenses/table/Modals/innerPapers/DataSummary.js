...
const DataSummary = () =>{
	...
	let ExpenseAmount = cur + Array(1).fill('\xa0').join('') + Num2(+value.Amnt);
	let Vat = cur + Array(1).fill('\xa0').join('') + Num2(value.Amnt-value.ExpAmntWthtoutVat);
	let ExpenseAmountBeforeVat = cur + Array(1).fill('\xa0').join('') + Num2(value.ExpAmntWthtoutVat);
