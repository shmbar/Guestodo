...
export default function MonthsPicker(props) {
	...
	const slctMonths=(val)=>{
 		...
 		if(dates.start==='' && dates.end===''){
			...
			props.setValuePL({...props.valuePL, 'From': dateFormat(n1,'mmm-yyyy'), 'To': dateFormat(n2,'mmm-yyyy')});
		 	handleClose();
		} 
	}
  ...
  return (
    <div>
      <Button color="primary" style={{textTransform: 'none', fontSize: '16px'}} onClick={handleClickOpen} size="large"
       ...
       { props.valuePL.From==='' ? '' : dateFormat(props.valuePL.From, 'mmm-yyyy').toString().concat(' - ').concat(dateFormat(props.valuePL.To, 'mmm-yyyy'))}
      </Button>
      <Dialog
