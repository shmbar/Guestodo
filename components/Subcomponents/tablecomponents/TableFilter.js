        ...
        return(
            <div >
		  		<Menu  id="Columns-menu" anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleClose} className='colsFiltermenuTop'>
                    <div className='p-grid menuColFilter'>
                        <div className='p-col-6 menuColFilterTitle'>Filters</div>
						<div className='p-col-6 menuColFilterReset'><span style={{cursor: 'pointer'}} onClick={props.resetFltr} >Reset</span></div>
				  	</div>
					<ListItem role={undefined} dense button onClick={props.runFltr} className="noPaddinTopBot" style={{paddingLeft: '7px'}}>
                        <Checkbox  checked={props.showFilter} tabIndex={-1}  /*disableRipple*/  /> 
						<ListItemText primary={'Filter Options'} style={{textAlign: 'left'}}  className='txtFont noPaddRightLEft' />
                    </ListItem> 
				</Menu>
                <Tooltip title="Filter Table" leaveTouchDelay={0}>
