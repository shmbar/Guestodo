  ...
  return (
    <div>
      <TextField
		  ...
		  fullWidth
		  value={props.value.AccDate===null ? '' : dateFormat(props.value.AccDate,'mmmm yyyy')}
		  InputProps={{
				readOnly: true,
			  	endAdornment: (
			...
			<Grid  container >
				{
					months.map((x,i)=>{
						return <Grid key={i} item xs={4} className={classes.cell}
								   style={(props.value.AccDate===null &&  dateFormat(new Date() ,'m')*1===i+1 &&  dateFormat(new Date() ,'yyyy')*1===yr) ? {border: '1px solid'}:
										 (props.value.AccDate!==null &&  dateFormat(props.value.AccDate ,'m')*1===i+1 &&  dateFormat(props.value.AccDate ,'yyyy')*1===yr) ?
